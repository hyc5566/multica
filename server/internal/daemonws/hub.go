...
type HeartbeatHandler func(ctx context.Context, identity ClientIdentity, runtimeID string, supportsBatchImport bool, ccxray json.RawMessage) (*protocol.DaemonHeartbeatAckPayload, error)
...
func (c *client) handleHeartbeatFrame(raw json.RawMessage) {
	...
	ack, err := handler(context.Background(), c.identity, payload.RuntimeID, payload.SupportsBatchImport, payload.CCXRay)
	if err != nil {
