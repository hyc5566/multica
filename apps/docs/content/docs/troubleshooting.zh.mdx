...
1. Agent 绑定的运行时是否在线；
2. 运行时是否检测到了该 Agent 配置的 AI 编程工具；
3. Agent 是否还有可用并发额度；
4. 守护进程是否还有全局执行容量。

Agent 默认最多同时执行 6 次运行；单个守护进程默认最多同时执行 20 次。达到上限时，新运行会留在队列中，等已有执行结束后再开始。运行时离线时，运行也会继续排队；只有该运行时心跳缺席超过重连宽限期、且该运行自己也已等满这么久时才会失败；所以排在忙碌运行时后面的长队列不会因为等太久而被判失败，指派给已经离线的机器时也仍然留有完整一个宽限期把它唤醒。
...
这表示另一条在途运行正在使用同一个本地目录。Multica 会等待目录锁释放，避免两个 Agent 同时修改同一份文件。
...
否则通常只需等前一次运行结束。若前一次已经卡住，可以从它的执行日志中停止；也可以为当前 Agent 选择其他本地目录。这个目录互斥由守护进程在内存中维护，不写磁盘锁文件；怀疑锁状态异常时运行 `multica daemon restart` 即可释放，没有需要手动删除的文件。
...
- 工具是否已经完成登录；
- API key、额度或模型权限是否可用；
- Agent 选择的模型和思考级别是否被该工具支持；
- 本地工作目录是否存在并且可写；
- Agent 的自定义参数或环境变量是否有效。
