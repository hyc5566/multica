...
<Callout type="info">
**GitHub 连接**决定 Multica 从哪些仓库接收 PR 事件；**代码仓库**设置决定 Agent 运行时可以选择哪些仓库。两者用途不同，需要分别配置。
</Callout>
...
| Co-authored-by | 在 Agent 创建的 commit 中加入 `Co-authored-by: multica-agent <github@multica.ai>`。 |
...
- [项目资源](/project-resources) — Agent 执行时使用哪些仓库。
