---
title: 自动化
description: 按时间或 Webhook 自动把重复工作交给 Agent。
---

import { Callout } from "fumadocs-ui/components/callout";

自动化用来执行反复发生的工作，例如每天汇总进展、定期检查依赖，或在外部系统发来事件时启动 Agent。

每条自动化保存一份 Runbook、一个执行方和一个或多个触发器。触发后，Multica 会创建任务或直接运行 Agent，并保留每一次运行记录。
...
- **名称**：说明这条自动化负责什么；
- **Runbook**：Agent 每次运行时读取的目标、背景、约束和步骤；
- **执行方**：一个 Agent 或小队；
...
Payload 会保存在投递和运行记录中，并交给 Agent。创建任务模式还会把事件内容附在任务描述中。
...
- 能管理自动化不代表一定能运行它使用的 Agent，Agent Access 仍然生效。
...
- [分配任务给 Agent](/assigning-issues) — 分配与确认窗口。
