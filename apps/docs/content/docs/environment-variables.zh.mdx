...
这组配置用于服务端的辅助生成能力，例如对话标题；它不是 Agent 运行时使用的 AI 编程工具凭据。
...
- **后续提问建议**（Agent 回复下方的按钮）—— 发送对话末尾的内容：最多 6 条消息，其中被追问的那条回复上限 3000 字符，更早的每条上限 800 字符。
...
<Callout type="info">
这只覆盖辅助生成这一层。Agent 执行是另一条数据路径：Agent 回复对话时，由你的守护进程调用该 Agent 的 AI 编程工具，用的是那个工具自己的凭据，守护进程不会把上面的 `MULTICA_LLM_*` 配置传给它。（Agent 自身需要的运行级 Multica 连接变量由守护进程单独注入。）把上面的变量留空不影响这条路径——它由 Agent 的运行时配置决定。
</Callout>

## 守护进程配置

下面的变量在运行 Agent 的电脑上读取，不是在 API 容器中读取。
