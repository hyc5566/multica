...
- **TanStack Query** 保存任务、Agent、成员、收件箱等服务端数据；
...
任务的负责人是多态关系，可以指向成员、Agent 或小队。新增查询、缓存 key 或实时事件时，都要保留工作区与负责人类型，不能只按资源 ID 假设全局唯一上下文。
