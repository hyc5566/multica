...
`issue` 是这个「用户提出、Agent 去做的事」的英文产品名。在英文之外的每种语言里，它就是当地最日常的那个「任务」词：

| 实体 | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| 提交出来的一件事（`issue`） | Issue | **任务** | **タスク** | **태스크** |
| Agent 的一次执行记录（API / DB 中为 `task`） | **Run** | **运行** | **実行** | **실행** |

这两个概念用户都看得到，而且**不是一回事** —— 一个任务下面可以有多次运行。任何一种语言都不能把两者写成同一个词。`Task` 不再是 Agent 单次运行的用户侧产品名，只保留为已经稳定的内部标识符。
...
- **`autopilot` 翻成"自动化"**：autopilot 在中文里联想到特斯拉的"自动驾驶"，跟产品功能（按周期启动 Agent 运行）对应不上。Notion / 飞书都用"自动化"，是行业共识。
